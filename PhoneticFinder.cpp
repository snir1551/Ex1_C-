    ...
    std::string find(std::string text, std::string word)
    {
        ...
        while(text[i] != '\0') // not end of the string text
        {
            ...
            else // text[i] != ' '
            {
                ...
                if(are_words_equal(text, word, i, endWordIndex)) // our function is checking start index to end index for one word, if found the word return true
                {
        ...
        //not found the word
        // std::string("Did not find the word ") + word + std::string(" in the text")
